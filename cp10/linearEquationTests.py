...
class LinearEquationTests(unittest.TestCase):
    ...
    def test_3_unknowns(self):
        ...
        def e3(genes):
            x, y, z = genes
            return 2 * z * fractions.Fraction(6, x) + 3 * fractions.Fraction(
                y, 2) - 6
    ...
    def test_4_unknowns(self):
        ...
        def e1(genes):
            ...
            return fractions.Fraction(
                1, 15) * x - 2 * y - 15 * z - fractions.Fraction(4, 5) * a - 3

        def e2(genes):
            x, y, z, a = genes
            return -fractions.Fraction(5, 2) * x - fractions.Fraction(
                9, 4) * y + 12 * z - a - 17

        def e3(genes):
            x, y, z, a = genes
            return -13 * x + fractions.Fraction(
                3, 10) * y - 6 * z - fractions.Fraction(2, 5) * a - 17

        def e4(genes):
            x, y, z, a = genes
            return fractions.Fraction(1, 2) * x + 2 * y + fractions.Fraction(
...
def display(candidate, startTime, fnGenesToInputs):
    ...
    symbols = "xyza"
    result = ', '.join("{0} = {1}".format(s, v) for s, v in zip(
        symbols, fnGenesToInputs(candidate.Genes)))
